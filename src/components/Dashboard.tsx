...
import { TrendingUp, ArrowRight, CheckCircle2, RotateCcw, Plus, Receipt, DollarSign, Wallet, Activity, CreditCard } from 'lucide-react';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div className="grid-summary">
        ...
        <div className="glass-card summary-card">
          ...
          <div className="summary-footer">
            <span>{settlements.length} completed settlement{settlements.length === 1 ? '' : 's'}</span>
          </div>
        </div>

        <div className="glass-card summary-card">
          <div className="summary-card-header">
            <span className="summary-title">Average / Member</span>
          ...
          <div className="summary-footer">
            <span>Equal 3-way split average</span>
          </div>
        </div>
      </div>
            ...
            return (
              <div key={user.id} className="glass-card user-balance-card">
                ...
                <div className="balance-status-box">
                  ...
                  <div
                    className="balance-val tabular-nums"
                    style={{
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.72rem', color: 'var(--text-muted)', marginBottom: '4px' }}>
                    <span>Paid vs Target Share</span>
                ...
                <div className="user-stats-grid">
                  <div className="stat-item">
                    <span className="stat-item-label">Total Out-of-pocket</span>
                    <span className="stat-item-value tabular-nums">{formatCurrency(b.totalPaidCents)}</span>
                  </div>
                  <div className="stat-item">
                    <span className="stat-item-label">Calculated Fair Share</span>
                    <span className="stat-item-value tabular-nums">{formatCurrency(b.totalShareCents)}</span>
                  </div>
                </div>
              </div>
            );
      ...
      <div className="glass-card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <h2 style={{ fontSize: '1.2rem', fontWeight: 800, letterSpacing: '-0.02em' }}>Optimal Settlement Plan</h2>
            <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>
              Algorithms compute minimal direct payments to settle all housemate balances
            </p>
          </div>
          <button className="btn btn-secondary btn-sm" onClick={onNavigateToSettlement}>
            <span>Full Settlement Details</span>
            <ArrowRight size={16} />
          </button>
        </div>
      ...
      <div>
        ...
        {recentExpenses.length === 0 ? (
          <div className="empty-state glass-card">
            ...
            <p style={{ fontSize: '0.85rem' }}>Click below to add your first expense split.</p>
            <button className="btn btn-primary" onClick={onOpenAddExpense}>
