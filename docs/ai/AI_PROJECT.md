...
- Production uses Firebase Authentication and Cloud Firestore on the Spark plan. Firebase Storage is not initialized in the bound project and no callable Cloud Functions are deployed. Configured production builds are online-only; scoped LocalStorage remains a cache, not a durable cloud-write queue.
...
- Both GitHub Pages and Vercel are active public deployment targets and have served the same production bundle. Verify both after each release because they deploy independently from `main`.
...
- The production project is `home-finance-1ah277j9`, uses the Firestore free tier in `asia-south1`, and has no deployed Functions or initialized Storage. `npm run audit:production` creates a private, git-ignored Firestore backup and reports aggregate integrity findings.
