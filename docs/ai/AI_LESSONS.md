...
## 2026-08-10 ? Legacy data and optional-field authorization
- FACT: A document can have the correct field names and wire types yet still fail every update because strict Rules validate the complete resulting document. A legacy 351,254-character inline avatar exceeded the current 40,000-character profile/roster limit and blocked otherwise valid wallet and house writes.
- FACT: Optional text fields must be omitted when blank. The personal-expense form sent `notes: ''`, but the Rules correctly accept either an absent note or a bounded non-empty note.
- DECISION: Normalize oversized legacy avatars in the browser on authenticated profile load and persist the compact photo to both profile and roster; omit blank personal notes. Do not weaken authorization or delete user data.
- Evidence: Production-shaped schema/identity audits; emulator regressions separating wallet failure from successful household ledger authorization; 60 frontend tests, 19 Rules tests, 10 backend tests, type-check, lint, build, and browser preview. Commit `f0f958b` passed GitHub Actions run `31403866730`; both production hosts served `assets/index-Cjci64QM.js` with HTTP 200 and the Vercel public shell had no captured console errors.
- Consequence: When valid-looking updates receive `permission-denied`, inspect size/range constraints across the entire post-write document and compare optional-field omission semantics, not only keys and authentication.
- Failed approach: Repeated broad Rules/schema fixes could not resolve these value-level incompatibilities because the failing legacy avatar and empty optional field were not visible in key-only audits.
